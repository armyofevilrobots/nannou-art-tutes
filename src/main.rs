        ...
        for y in 0..TILES_WIDTH {
            match _model.cells[(x + (y * TILES_WIDTH)) as usize] {
                Tile::UpRight => {
                    draw.line()
                        .start(pt2(
                            (x * TILE_SIZE - CENTER).into(),
                            ((y + 1) * TILE_SIZE - MIDDLE).into(),
                        ))
                        .end(pt2(
                            ((x + 1) * TILE_SIZE - CENTER).into(),
                            (y * TILE_SIZE - MIDDLE).into(),
                        ))
                        .weight(4.0)
                        .color(STEELBLUE);

                    // Turns out that drawing circles is VERY slow, so I turned it off
                    // draw.ellipse().color(STEELBLUE).w(4.0).h(4.0).x_y(
                    //     (x * TILE_SIZE - CENTER).into(),
                    //     ((y + 1) * TILE_SIZE - MIDDLE).into(),
                    // );
                    // draw.ellipse().color(STEELBLUE).w(4.0).h(4.0).x_y(
